# LUDI — Architecture & codebase map
...
- **v10 (LUDI):** `stretches`(++id,name,type,bodyArea,isCustom; +durationSec,description,difficulty), `stretchRoutines`(++id,name,phase,createdAt; +items[{stretchId,durationSec}],bodyArea,isCustom), `stretchLogs`(++id,date,phase,completedAt,workoutId; +routineId,routineName,durationSec)
...
## localStorage keys
- **`ludi_prefs`** (settingsStore; falls back to the legacy `opus_prefs` once on load so existing installs keep units/equipment/onboarding): barWeight, unit, onboarded, effects, sound, theme, tourSeen, restDuration, stepGoal, waterGoal, recapDismissedWeek, coachMarksSeen, inventory{active,gym/home:{barKg,plates,unit}}, **reminderEnabled, reminderHour, lastRemindedDate, stretchPrompts**
