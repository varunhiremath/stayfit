...
- workout: WorkoutPage, ExerciseSection, SetLogger, CardioLogger, RestTimer, PlateCalculator, ExercisePicker, EndWorkoutModal (+ cool-down prompt), WorkoutCard, ExerciseInfoModal, **ExerciseDemo** (collapsible how-to picture + video, inline in the log), **SessionTimer** (dark elapsed-clock card + session progress), **SwapSuggestions** (same-muscle alternatives)
