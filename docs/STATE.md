...
**Library search + filter fixes (from user feedback).** Three problems, all reported after the hip exercises landed. (1) The **muscle filter was a horizontally scrolling strip** — 17 pills across 1342px inside a 310px window, so only the first five were ever visible and Adductors sat dead last where nobody found it. It now **wraps**, showing every group at once, and `LABEL`'s insertion order (which drives the pills) was resequenced to run push → pull → arms → core → legs so the leg muscles cluster together. (2) **Typing scrolled the results out of view**: the filter block unmounts on the first keystroke, which shortens the page under your thumb. `ExercisesTab` now pins the search row to the top of the viewport on the empty→non-empty transition, so matches appear directly beneath it. (3) **Search was a bare `name.includes(query)`** — a stray space, a hyphen, reversed words or one wrong letter all returned nothing. New pure `utils/exerciseSearch.js` (+20 tests) normalises accents/punctuation/whitespace, matches each word independently in any order, compares a space-squashed form (so "pushup" finds Push-Up), tolerates **one edit per word** for tokens of 4+ characters (short ones stay strict, or "abs" would match "arms"), and also searches muscle group and equipment. Results are ranked — whole-name prefix, then substring, then word matches, then fuzzy, then tag-only — so "adduction" lists the adduction moves before the typo-adjacent abduction ones. `useStretches` uses the same matcher, so both Library tabs behave identically.

**Abductor & adductor exercises (from user feedback).** Two new muscle groups — **`abductors`** (outer hip) and **`adductor`** (inner thigh) — taking the taxonomy from 15 tokens to 17. Both are `react-body-highlighter` slugs (the singular *adductor* is theirs, not a typo), verified against the library's model data: abductors render on the front view, adductor on the back, so the body map highlights them properly. **11 exercises:** Hip Abduction Machine, Cable Hip Abduction, Clamshell, Side-Lying Leg Raise, Lateral Band Walk / Hip Adduction Machine, Cable Hip Adduction, Lateral Lunge, Sumo Squat, Cossack Squat, Copenhagen Plank. Threaded through all 14 places that enumerate muscle groups — `ExerciseForm` (so custom moves can use them), `BodyPicker`, `RecoveryMap`, `useRecovery.ALL_MUSCLES`, four `MUSCLE_HUE` maps, `ExercisePicker`'s Legs filter, `weekPlanner.LEGS`, `routineName`, `dailyBriefing` — so generated leg days and the recovery map now include them rather than silently dropping them. **Data safety:** seed ids are positional (`i + 1`), so the block is **appended, never inserted** — a mid-list insert would renumber every later exercise and reattach logged sets to the wrong movement. New `seedExercises.test.js` (+7) pins that invariant with named id anchors. Existing installs back-fill through a new idempotent `ensureHipExercises()` (the `ensureCardioExercises` pattern) which lets Dexie auto-assign ids; verified on a simulated pre-change DB — the 11 rows arrive as ids 94–104, Bench Press keeps id 3, a logged set still resolves to Bench Press, and a second open adds no duplicates.

**Units moved onto the fields, plus live BMI (from user feedback).** Onboarding no longer opens with a standalone Units segment. Each measurement now carries its own unit picker on its label row: **Bodyweight** gets `kg | lbs` (still the app-wide weight unit) and **Height** gets `cm | ft/in`, which swaps the single centimetre box for a feet box and an inches box. A **BMI card sits directly under the two**, updating as you type — the figure in colour with its WHO band (Underweight / Healthy / Overweight / Obese). New pure `utils/bmi.js` (+16 tests): `toCm`, `toFeetInches` (12″ rolls over into the next foot, so never 5′12″), `fmtHeight`, `calcBmi`, `bmiCategory`. Height is still **stored in cm** whichever way it was entered; the new `heightUnit` pref only picks the display, and Profile's identity line and the Settings height row both honour it — Settings gained the same `cm | ft/in` toggle and its own BMI row, so the choice doesn't read back wrong after setup.

**Dropped the energy check-in and the rest timer (from user feedback).** The workout tab now goes straight from the clock to the exercises. The "How's your energy today?" 1–5 row is gone, and with it `workoutStore.setEnergy`, the `energy` field on the active session and the `energyLogs` write in `completeWorkout` — the **table stays declared** (append-only) and `deleteWorkout` still clears legacy rows, so nobody's old data is orphaned. The post-set `RestTimer` card is gone too, along with `components/workout/RestTimer.jsx`, the `restDuration`/`setRestDuration` pref, and the whole `onSetLogged` callback chain that existed only to trigger it (`WorkoutPage` → `ExerciseSection` → `SetLogger`/`CardioLogger`). Rest is still *reported* in history — `utils/restStats.js` derives it from set timestamps, which never needed the widget. Swept up one more RPG leftover found in passing: `CardioLogger` was still stamping a `bonusXp` field on every cardio bout.

**Stripped the session card back to logging (from user feedback).** Five changes, all subtractive except the timing. (1) The **target line ("4×8") and the green progress bar are gone** from the exercise card — you do as many sets as the day allows, and the card just tallies what you actually did. (2) **RPE is gone** entirely: the chip row, the "+ Add effort" toggle, the explainer and the per-set badge (the `sets.rpe` column stays for old history and the CSV export). (3) The session timer's right-hand stat now reads **"N of M done" — exercises completed, not sets**. (4) **Nothing opens by itself.** The session lands as a plain list of names; a card opens only when you tap it, and opening one closes the previous. (5) The **link/superset controls are gone** (`utils/supersets.js` deleted with its tests, `toggleSuperset` removed from the store), so every exercise is just an exercise. New in the same pass: **per-exercise time**. `WorkoutPage` banks the seconds a card was open (`workoutStore.accrueExerciseTime`, live-ticking while it is open), the Done row shows that split next to a clock icon, and `completeWorkout` persists the whole map as an unindexed `workouts.exerciseTimes` (no migration needed). Onboarding also lost its **empty-barbell-weight field** — a leftover of the removed setting that called a `setBarWeight` action that no longer exists, so "Begin" would have thrown on a fresh install.

**Focused workout screen + aligned nav (from user feedback).** Four changes. (1) The centre nav button now uses **`PulseIcon`** — the app icon's ring-and-wave reduced to two shapes, because the previous lifter packed ten thin strokes into 26px and blurred. (2) It is **no longer raised**: every tab shares one 36px icon slot so all five icons and labels sit on a common baseline, with Workout emphasised by a filled emerald circle instead of by height. (3) A dark **`SessionTimer`** card puts the elapsed clock at the top of the session in large tabular monospace, with a stopwatch badge, live sets/volume, a breathing dot and a session progress bar. (4) The session is now an **accordion**: exactly one exercise is expanded for logging while the rest collapse to single rows, unstarted rows offer **same-muscle swap chips** (`SwapSuggestions`) so you can trade an exercise for an alternative, and anything with logged sets folds into a **Done** section once you move on. New pure `utils/sessionFlow.js` (+22 tests) owns the todo/done split, the auto-advance and the swap candidates. **Removed with this:** the per-exercise up/down reorder arrows (the accordion supersedes manual ordering) and the old swap-via-picker path, replaced by the inline chips.

**How-to media in the workout log + tappable exercise names (from user feedback).** Each exercise in an active session now has an inline **"How to do it"** row directly under its name: collapsed it is one thin strip with a thumbnail, expanded it shows the demonstration picture and a link to a form video, so the log stays scannable for a six-exercise session. The wger image lookup that only lived on the exercise detail page was extracted into a shared **`useExerciseDemo`** hook (in-memory cached, fails silently offline) and reused on the detail page, in the info sheet and in the log. Added a `wger.de/media` runtime cache to the service worker so a picture you have already seen still loads at the gym with no signal. Separately, the small **(i) button was removed** — the **exercise name itself is now the link** into its info sheet, underlined in the accent colour.

**New app icon — "Pulse" (from user feedback).** Six directions mixing strength and stretch were presented; the user chose **Pulse**: a weight-plate ring with a movement wave running through it, in the app's emerald on white. `BrandMark` was redrawn to match, so the in-app mark, the splash and the installed icon are now the same artwork. Generated `icon-192`, `icon-512`, a proper full-bleed `icon-maskable-512` (artwork inset to Android's safe zone), `apple-touch-icon` (180), `favicon-32` and a scalable `icon.svg`, all wired into `index.html` and the manifest; notifications use the 192. Deleted the inherited 1 MB `lifter.png` photo — **precache fell from 2,650 KiB to 1,084 KiB**.
